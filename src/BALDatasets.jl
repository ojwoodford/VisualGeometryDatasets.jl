...
const balgroups = Dict("dubrovnik"=>dubrovnik, "trafalgar"=>trafalgar, "ladybug"=>ladybug, "venice"=>venice)

function getinfo(name::String)
for group = keys(balgroups)
    for col = eachcol(balgroups[group])
        if name == col[1]
            return (group, string(col[1], "-pre.txt.bz2"), col[2])
        end
    end
end
