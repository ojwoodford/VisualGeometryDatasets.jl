...
struct BALMeasurement{T<:AbstractFloat}
    x::T
    y::T
    camera::Int
    landmark::Int
...
struct BALDataset{T<:AbstractFloat}
    measurements::Vector{BALMeasurement{T}}
    cameras::Vector{SVector{9, T}}
    landmarks::Vector{SVector{3, T}}
...
function readbalfile(fpath::String, T::Type=Float64)
    open(fpath, "r") do fh
        ...
        # Init the output
        output = BALDataset{T}(nummeasurements, numcameras, numlandmarks)

        # Read in the measurements
        for ind = 1:nummeasurements
            camera, landmark, x, y = split(readline(stream))
            output.measurements[ind] = BALMeasurement{T}(parse(T, x), parse(T, y), parse(Int, camera)+1, parse(Int, landmark)+1)
        end
        ...
        start = 0
        for ind = 1:numcameras
            output.cameras[ind] = variables[StaticArrays.SUnitRange(1, 9) .+ start]
            start += 9
        end

        # Initialize the landmarks
        for ind = 1:numlandmarks
            output.landmarks[ind] = variables[StaticArrays.SUnitRange(1, 3) .+ start]
            start += 3
        end
